...
class ReservationStep2View(View):
    ...
    def get(self, request):
        ...
        # Release expired reservations (older than 15 minutes in phone_verified status)
    ...
    def post(self, request):
        ...
        # Release expired reservations before checking
        self.release_expired_reservations()
...
class ReservationStep4View(View):
    ...
    def get(self, request, tracking_code):
        ...
        # Check if reservation is expired (more than 15 minutes old and not paid)
        if reservation.status == 'phone_verified' and reservation.payment_status == 'unpaid':
            if reservation.phone_verified_at:
                time_elapsed = timezone.now() - reservation.phone_verified_at
                if time_elapsed > timedelta(minutes=15):
                    # Release the slot and cancel reservation
                    reservation.status = 'cancelled'
                    reservation.save()
                    ...
                    messages.error(request, '⏰ زمان رزرو شما به پایان رسیده است. لطفاً مجدداً نوبت بگیرید.')
                    return redirect('council:step1_initial')
